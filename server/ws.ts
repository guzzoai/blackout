...
interface PowerupState {
  ...
  id: number;
  active: boolean;
  respawnAt: number; // timestamp when it should reappear, 0 if active
}
...
function generateMazeGrid(seed: number): { grid: boolean[][]; spawn1: { x: number; y: number }; spawn2: { x: number; y: number } } {
...
function startGame(room: Room) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    send(room.players[i]!.ws, { t: 'powerups', list: room.powerups.filter(p => p.active).map(p => ({ x: p.x, y: p.y, id: p.id })) });
  }

  // Start tick loop for bullet sim
...
function regenerateMap(room: Room) {
  ...
  // Respawn players at new spawn points
  for (let i = 0; i < 2; i++) {
    const p = room.players[i];
    if (!p) continue;
    const spawn = i === 0 ? spawn1 : spawn2;
    ...
    p.y = spawn.y;
    p.hp = MAX_HP;
    p.invulnUntil = Date.now() + RESPAWN_INVULN_MS;
  }

  // New powerups
  generatePowerups(room, grid);

  // Send new map to both
  for (let i = 0; i < 2; i++) {
    const p = room.players[i];
    if (!p) continue;
...
function tickRoom(room: Room) {
  ...
  room.bullets = room.bullets.filter(b => {
    ...
    // Hit detection — check against the other player
    const targetIdx = b.owner === 0 ? 1 : 0;
    const target = room.players[targetIdx];
    if (!target) return true;

    const now = Date.now();
    if (target.invulnUntil > now) return true;
    ...
    if (dist < BULLET_HIT_DIST) {
      ...
      if (target.hp <= 0) {
        ...
        for (const p of room.players) {
          if (p) send(p.ws, { t: 'kill', killer: b.owner, victim: targetIdx, s1, s2 });
        }
        ...
        if (s1 >= KILLS_TO_WIN || s2 >= KILLS_TO_WIN) {
          ...
          for (const p of room.players) {
            if (p) send(p.ws, { t: 'win', winner });
          }
          room.phase = 'over';
          if (room.tickInterval) clearInterval(room.tickInterval);
        } else {
          // Generate new map after kill
        ...
        // Hit but not dead
        for (const p of room.players) {
          if (p) send(p.ws, { t: 'hit', target: targetIdx, hp: target.hp });
        }
        return false;
      }
    }
  ...
  // Check powerup respawns — pick a new random open tile
  const now2 = Date.now();
  for (const pu of room.powerups) {
    if (!pu.active && pu.respawnAt > 0 && now2 >= pu.respawnAt) {
      // Find a random open cell for the new location
      ...
      for (let y = 3; y < ROWS - 3; y++) {
        for (let x = 3; x < COLS - 3; x++) {
          if (!room.grid![y][x]) {
            ...
            // Avoid spawning on top of other active powerups
            const tooClose = room.powerups.some(other => other.active && Math.abs(other.x - cx) < TILE * 2 && Math.abs(other.y - cy) < TILE * 2);
      ...
      for (const p of room.players) {
        if (p) send(p.ws, { t: 'powerup_spawn', x: pu.x, y: pu.y, id: pu.id });
      }
    }
  }
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (p && room.players[i]) {
      send(room.players[i]!.ws, { t: 'pos', x: p.x, y: p.y, a: p.a });
    }
  }
}
    ...
    switch (msg.t) {
      case 'create': {
        ...
        const room: Room = {
          code,
          players: [
            { ws, x: 0, y: 0, a: 0, hp: MAX_HP, score: 0, invulnUntil: 0 },
            null,
          ],
          bullets: [],
          seed: 0,
      ...
      case 'shoot': {
        ...
        // Check bullet limit
        const existing = room.bullets.filter(b => b.owner === idx).length;
        if (existing >= MAX_BULLETS_PER_PLAYER) return;
        ...
        // Broadcast shoot to opponent
        const other = 1 - idx;
        if (room.players[other]) {
          send(room.players[other]!.ws, { t: 'shoot', x: msg.x as number, y: msg.y as number, a });
        }
        break;
      }
      case 'pickup': {
        ...
        // Only the grabber gets the reveal effect
        send(player.ws, { t: 'powerup_taken', id: puId, by: idx });
        // Opponent just sees the powerup disappear
        const otherIdx = 1 - idx;
        if (room.players[otherIdx]) {
          send(room.players[otherIdx]!.ws, { t: 'powerup_gone', id: puId });
        }
        break;
      }
    }
